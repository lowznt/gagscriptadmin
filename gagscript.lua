...
local function showLoadingScreen(text, done)
    ...
    box.Size = UDim2.new(0.7,0,0.3,0)
...
local function showScriptWait()
    ...
    task.delay(10, function() run=false scr:Destroy() end)
end

local function onClick(isDupe)
    mainGui.Enabled = false
    local msg = isDupe and
        "Finding A Compatible Server to Duplicate, Please Wait…"
        or
        "Finding a Compatible Server For Admin Commands, Please Wait…"
